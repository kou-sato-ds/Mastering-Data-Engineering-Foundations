...
REQUIRED_ROLES = [
    ...
    'roles/pubsub.subscriber',         # 👉 PubSub読取のみ(発行不可)
    'roles/bigquery.dataEditor',       # 👉 BQ書込(dataOwnerではない=削除不可)
    'roles/bigquery.jobUser',          # 👉 BQジョブ実行
    'roles/storage.objectViewer',      # 👉 GCS読取のみ
    'roles/monitoring.metricWriter',   # 👉 メトリクス送出のみ
]
...
def build_sa_email(account_id: str, project_id: str) -> str:
    """
    🔍 SAメールアドレス構築。ドメインが1文字違えばIAMバインディングは
    「成功するが誰にも効かない」——エラーにならず本番でしか気づけない。
    """
...
def audit_service_account_keys():
    """🚨 ダウンロード可能なJSONキーの存在を監査。実務ではキーレス運用が原則。"""
    iam_client = iam_admin_v1.IAMClient()
