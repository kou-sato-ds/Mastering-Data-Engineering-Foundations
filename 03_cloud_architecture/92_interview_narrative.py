...
NARRATIVES = [
    ...
    },
    {
        ...
        'answer': (
            '変更を後方互換と破壊的に分類し、バージョン番号と一致するか検証します。'
            '型の緩和は許可リストで宣言し、そこに無い遷移は全て破壊的とみなします——'
            '禁止リストにすると、新しい型が追加されたとき自動的に安全と判定されるためです。'
            '列を追加した場合はバックフィル対象として返し、'
            '「追加は安全」で終わらせて過去がNULLの列を作らないようにしています。'
        ),
        'evidence': [105],
        'sibling': 'ADR-012',
    ...
    {
        'id': 'backfill_planning',
        'question': '過去データを作り直すときに気をつけることは何ですか',
        'answer': (
            '一度に全期間を流さないことです。'
            '30日分を1ジョブで流して28日目に失敗すれば27日分が無駄になるため、'
            '日次チャンクに分けて同時実行数も制限します。'
            'バックフィルは本番と同じクォータを使うので、'
            '急ぐ作業のために動いている仕組みを壊さないためです。'
            '実行前にコスト見積も出します——90日分は日常的な依頼ですが、'
            '1日50GBなら4.5TBになり、請求で初めて規模を知ることになります。'
        ),
        'evidence': [103],
        'sibling': 'ADR-013',
        'concern': 'backfill',
